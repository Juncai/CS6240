...
We think the distance is a better feature than air time when predicting the ticket price. From all the coefficients above, we can see that $\theta_{1}$ is always much larger then the $\theta_{0}$ which means the features always contribute much more to the result rather than the contstant does. The other observation is that, the $\theta_{1}$ for distance is always larger than $\theta_{1}$ for air time model; and the $\theta_{0}$ for distance model is always smaller than $\theta_{0}$ for the air time model from which we conclude that distance is a better explanatory varialble here.
