...
res <- subset(rawRes, select = -c(originAI, originCity, destAI, destCity, flNum, flDate, crsDepTime))
...
#	print(paste(paste(rawRes$originAI[i], rawRes$originCity[i], rawRes$destAI[i], sep="_", collapse=NULL), prediction[i], sep=",", collapse=NULL))
	write(paste(paste(rawRes$flNum[i], rawRes$flDate[i], rawRes$crsDepTime[i], sep="_", collapse=NULL), prediction[i], sep=",", collapse=NULL), file=output, append=TRUE)
}
