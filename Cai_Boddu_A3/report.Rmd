...
There two datasets being used in our tests. The smaller one only contains one data file which is 298.csv.gz. The larger one contains all the data files given in the homework 1.(all-v1.tar)

<h3>Conclusion</h3>
In the analysis of our graph we can draw upon many conclusions from this project. 

As expected, the multi-threaded implementations showed performance gains over their single-threaded counterparts. 

It can also be seen that the duration of fast median is almost an exact copy of the true median. This shows that our implementation of fast median is not an ideal one.

Another conclusion is that our EMR implementations stand out compared to the others i.e. EMR is extremely slow comparatively. This can be explained due to the large overhead in setting up a cluster on EMR. Perhaps if our data set was multiple factors larger, then we could see performance benefits on this implementation. 

Similarly, the pseudo-distributed mode took longer as well compared to our single and multi threaded implementations. This could be explained due to the overhead of hadoop as well.
