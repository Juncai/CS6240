...
pd () {
	...
	hadoop jar build/libs/ClusterAnalysis.jar "$mainClass" input output
...
emr () {
	...
	if [ "$mainClass" == 'mean.ClusterAnalysis' ]; then
		...
		cid=$(aws emr create-cluster --applications Name=Hadoop \
			...
			--log-uri ${loguri} \
			--steps '[{"Args":["ClusterAnalysis","s3://juncai001/input","s3://juncai001/output"],"Type":"CUSTOM_JAR","ActionOnFailure":"CONTINUE","Jar":"s3://juncai001/ClusterAnalysis.jar","Properties":"","Name":"mean.ClusterAnalysis"}]' \
			--name 'Jun MR cluster' \
		...
		cid=$(aws emr create-cluster --applications Name=Hadoop \
			...
			--release-label emr-4.3.0 \
			--log-uri ${loguri} \
			--steps '[{"Args":["ClusterAnalysis","s3://juncai001/input","s3://juncai001/output"],"Type":"CUSTOM_JAR","ActionOnFailure":"CONTINUE","Jar":"s3://juncai001/ClusterAnalysis.jar","Properties":"","Name":"median.ClusterAnalysis"}]' \
			--name 'Jun MR cluster' \
			--instance-groups '[{"InstanceCount":1,"InstanceGroupType":"MASTER","InstanceType":"m1.medium","Name":"Master Instance Group"},{"InstanceCount":2,"InstanceGroupType":"CORE","InstanceType":"m1.medium","Name":"Core Instance Group"}]' \
			--configurations '[{"Classification":"spark","Properties":{"maximizeResourceAllocation":"true"},"Configurations":[]}]' \
			--auto-terminate \
...
mainClass='mean.ClusterAnalysis'
...
if [ $1 = '-median' ]; then
	mainClass='median.ClusterAnalysis'
fi
