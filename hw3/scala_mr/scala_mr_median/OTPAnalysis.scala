...
//      var prices:java.util.List[java.lang.Double] = new java.util.ArrayList[java.lang.Double]()
      ...
      if (isActiveCarrier(values)) {
        ...
        monthPrices.keySet.foreach(mk => {
          ...
          median.set(getMedian(monthPrices(mk).toList))
          ctx.write(carrierMonth, median)
        })
      }
  ...
  def getMedian(prices: List[Double]): Double = {
    val lop = prices.sorted
    val len = lop.length
    if (len % 2 == 0) {
        return (lop(len / 2) + lop((len / 2) - 1)) / 2;
    } else {
        return lop(len / 2);
    }
  }
